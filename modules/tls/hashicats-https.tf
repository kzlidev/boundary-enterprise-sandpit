...
resource "local_file" "hashicats_private_key" {
  ...
  filename = "${path.root}/tmp/tls/hashicats_private_key.key"
}
...
resource "tls_cert_request" "hashicats_csr" {
  ...
  dns_names = ["http.boundary", "https.boundary", "*.boundary", var.route53_hashicats_hosted_zone_name]
...
resource "local_file" "hashicats_cert" {
  ...
  filename = "${path.root}/tmp/tls/hashicats.cert"
}
